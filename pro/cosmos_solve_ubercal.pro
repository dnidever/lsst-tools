...
pro cosmos_solve_ubercal,overlapstr,ubercalstr,matchthresh=matchtresh,errthresh=errthresh,iterthresh=iterthresh,verbose=verbose
...
sz = size(overlapstr.visit)
nchips = sz[1]

; Thresholds
if n_elements(matchthresh) eq 0 then matchthresh=10
if n_elements(errthresh) eq 0 then errthresh=0.07
if n_elements(iterthresh) eq 0 then iterthresh=300

; Initialize ubercal structure
ubercalstr = replicate({visit:'',ccdnum:-1L,magoff:0.0,magofferr:0.0,noverlap:-1L,flag:-1},nchips)
ubercalstr.visit = overlapstr.visit
ubercalstr.ccdnum = overlapstr.ccdnum
...
WHILE (flag eq 0) do begin
  ...
  for k=0,nchips-1 do begin
    ...
    if overlapstr.noverlap[k] gt 0 then begin
      ...
      gd = where(data[ind].nmatch gt matchthresh and data[ind].magofferr lt errthresh,ngd)
      if ngd eq 0 then $  ; raise the error threshold slightly
        gd = where(data[ind].nmatch gt matchthresh and data[ind].magofferr lt errthresh+0.02,ngd)
      ...
      if ngd gt 0 then begin
        ...
        if keyword_set(verbose) then print,count,k+1,ubercalstr[k].visit,ubercalstr[k].ccdnum,ngd,robmean,robsig,$
    ...
    if ngd eq 0 then begin
      if keyword_set(verbose) then print,count,k+1,ubercalstr[k].visit,ubercalstr[k].ccdnum,ngd,format='(I5,I5,A11,I5,I7)'
  ...
  ; Do we need to stop
  if count ge iterthresh or (abs(davg-davg0)/davg0 lt 1e-2 and abs(dmax-dmax0)/dmax0 lt 1e-2) then flag=1
...
uiexp = uniq(ubercalstr.visit,sort(ubercalstr.visit))
uexp = ubercalstr[uiexp].visit
...
for k=0,nuexp-1 do begin
  ...
  expindgd = where(ubercalstr.visit eq uexp[k] and ubercalstr.magoff lt 50,nexpindgd)
  ...
  expindall = where(ubercalstr.visit eq uexp[k],nexpindall)
  if nexpindall gt 0 then rel_magoff[expindall] -= expmagoff[k]
endfor
; For unique chips measure the median relative offset
uiccdnum = uniq(ubercalstr.ccdnum,sort(ubercalstr.ccdnum))
uccdnum = ubercalstr[uiccdnum].ccdnum
nuccdnum = n_elements(uccdnum)
ccdnumdeltamagoff = fltarr(nuccdnum)
ccdnumdeltamagofferr = fltarr(nuccdnum)+9.99
for k=0,nuccdnum-1 do begin
  chind = where(ubercalstr.ccdnum eq uccdnum[k],nchind)
  if nchind gt 0 then begin
    ccdnumdeltamagoff[k] = median([rel_magoff[chind]])
    ccdnumdeltamagofferr[k] = mad([rel_magoff[chind]]) / sqrt(nchind)  ; error in mean
  endif
endfor
...
for k=0,nbdind-1 do begin
  expind = where(uexp eq ubercalstr[bdind[k]].visit,nexpind)
  chind = where(uccdnum eq ubercalstr[bdind[k]].ccdnum,nchind)
  if nexpind gt 0 and nchind gt 0 then begin
    ind = overlapstr.index[overlapstr.ind0[bdind[k]]:overlapstr.ind1[bdind[k]]]   
    ubercalstr[bdind[k]].magoff = expmagoff[expind] + ccdnumdeltamagoff[chind]
    ubercalstr[bdind[k]].magofferr = sqrt( expmagofferr[expind]^2 + ccdnumdeltamagofferr[chind]^2 )  ; add errors in quadrature
